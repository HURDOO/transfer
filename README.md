...
응답의 6자리 코드는 `GET /api/codes/:code`로 종류를 판별할 수 있다. 저장형이면 `GET /api/shares/:code`와 각 파일의 `downloadUrl`을 사용한다. 자세한 범위와 계약은 `docs/SPEC.md`, 구조는 `docs/ARCHITECTURE.md`를 따른다.
